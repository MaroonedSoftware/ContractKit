...
Generates server router files from `operation` declarations, targeting Koa or Fastify. Optionally also emits Zod schemas or plain TypeScript types from `contract` declarations (used for typing route handlers).
...
containing something like `/documents/<id>/content` becomes a parse error. The same applies to a
Docusaurus docs root, where that file would become another doc page.
...
A Docusaurus docs folder: one Markdown page per endpoint and per documented model, plus the
`_category_.json` files the autogenerated sidebar reads. Unlike the Mintlify pages these carry
their whole body, because there is no spec for Docusaurus to render from.
...
The folder needs nothing installed or configured on the site: the default autogenerated sidebar
picks it up, and a hand-written sidebar takes it as
`{ type: 'autogenerated', dirName: 'api-reference' }`.

Every page carries the `mdx.format: md` frontmatter, which opts it into CommonMark. Docusaurus
parses `.md` as MDX by default and would reject the `<details>` blocks and the `<br>` inside table
cells, so the opt-in is what lets these pages render with no site-wide `markdown.format` setting.

Model references are relative file links, so a broken one fails the site build rather than
rendering as a dead anchor. Endpoint and model grouping, page titles and slugs follow the same
rules as the Mintlify target. `index.md` is written only when absent, so it is yours to edit;
every other file is regenerated.
...
| Field             | Type      | Description                                                                                         |
| ----------------- | --------- | --------------------------------------------------------------------------------------------------- |
| `baseDir`         | `string`  | Output directory relative to `rootDir`. Default: `swift-sdk`                                        |
| `moduleName`      | `string`  | SwiftPM target and product name; sources land in `Sources/<moduleName>/`. Default: `ContractKitSdk` |
| `sdkName`         | `string`  | Aggregator class name. Must differ from `moduleName`. Default: `Sdk`                                |
| `includeInternal` | `boolean` | Whether to emit client methods for `internal` operations. Default: `false`.                         |
| `scaffold`        | `boolean` | Emit `Package.swift` once, as a user-owned file. Default: `false`.                                  |
