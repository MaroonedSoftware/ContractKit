...
- 7cb14cb: A generated MCP tool whose operation takes a query now hands its service the parsed query when the caller sends none.
...
- 6e6cbba: The generated router now writes `date` and `time` response fields in their contract format, so the SDK can read them back.
...
- a59e2f1: The generated TypeScript SDK no longer changes decimal.js's global settings when it is imported.
...
- 486b378: The generated SDK's default `X-Request-ID` now works in a browser page served over plain HTTP.
...
- 7f53618: Revive the fields a model inherits, so a child of a revivable base defines the reviver its callers import.
...
- 7a94596: Generated MCP output now enforces the security its contracts declare.
