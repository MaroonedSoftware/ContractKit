...
In SDK type files, a contract whose request shape holds a `date`, `time` or `decimal`, directly, through a base or through a contract it references, also gets a **`serializeModel()`** function. SDK methods pass a JSON or urlencoded request body through it before stringifying, so a `date` or `time` goes out in its contract `format` (`yyyy-MM-dd` and `HH:mm:ss` by default), which is what the router reads it with, rather than as `DateTime.toJSON()`'s full timestamp, and a `decimal` goes out in normal notation rather than `1e-8`. It returns a copy and leaves the caller's object as it was. A body that is not a plain contract reference, such as an inline object, gets the same treatment from a function in the client file.

Server type files get a **`serializeModel()`** too, for the other direction. It covers a contract whose response shape holds a `date` or `time`, directly, through a base or through a contract it references, and takes what the service returns (`ModelOutput` where `format(output=…)` re-keys it). The router passes the service result through it, so a `date` or `time` goes out in its contract `format`, the one every SDK reads it back with. It leaves a `decimal` alone, since the server's decimal.js configuration already keeps its `toJSON()` in normal notation.
...
A JSON response body that holds a `date` or `time` is written through the response serializer its types file declares: `ctx.body = serializeRelease(result)` (or `reply.send(...)`), `result.map(serializeRelease)` for an array, and a function in the router file for a body that is not a plain contract reference, such as an inline object or a bare `date`. Both frameworks otherwise stringify a luxon `DateTime` with its `toJSON()`, a full timestamp such as `2026-09-23T00:00:00.000Z`, which an SDK reading the field with `DateTime.fromFormat(v, 'yyyy-MM-dd')` rejects. The serializer composes with `validateResponses` and with `bigIntReplacer`, and a non-JSON mime is written as the service built it. A `date` or `time` response header is written with `toFormat()` in its contract format rather than `String()`. MCP tool results go through the same serializers. A router calls a contract's serializer only when `output.types` is generated, since a hand-written types file declares none.
