...
export function splitByTag(
  models: ModelNode[],
  routes: OpRouteNode[],
  routeTags: Map<OpRouteNode, string>,
): Map<string, CkRootNode> {
      ...
      for (const ref of refs) {
        if (modelNameToModel.has(ref)) {
          ...
          if (currentTag && currentTag !== 'shared') {
            ...
            const otherTags = [...modelsByTag.entries()]
              .filter(([t, r]) => t !== currentTag && r.has(ref))
              .map(([t]) => t);
 ...
 */
export function mergeIntoSingle(
  models: ModelNode[],
  routes: OpRouteNode[],
  filename: string = 'api',
): CkRootNode {
