...
describe('model serialization', () => {
  ...
  it('serializes model with descriptions as comments', () => {
    const root = ckRoot({
      models: [
        model('User', [
          field('id', scalarType('uuid'), { description: 'The user ID' }),
        ], { description: 'A user' }),
      ],
    });
    const result = astToCk(root);
  ...
  it('serializes model with mode and inputCase modifiers', () => {
    const root = ckRoot({
      models: [
        model('WebhookData', [
          field('event', scalarType('string')),
        ], { mode: 'loose', inputCase: 'snake' }),
      ],
    });
    const result = astToCk(root);
  ...
  it('serializes nullable fields', () => {
    const root = ckRoot({
      models: [
        model('Item', [
          field('parentId', scalarType('uuid'), { nullable: true, optional: true }),
        ]),
      ],
    });
    const result = astToCk(root);
...
describe('route serialization', () => {
  ...
  it('serializes route with internal modifier', () => {
    const root = ckRoot({
      routes: [
        opRoute('/webhooks/stripe', [
          opOperation('post', { responses: [opResponse(204)] }),
        ], { modifiers: ['internal'] }),
      ],
    });
    const result = astToCk(root);
