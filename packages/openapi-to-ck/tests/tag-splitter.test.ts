...
describe('splitByTag', () => {
  it('splits routes into separate files by tag', () => {
    ...
    const userRoute = opRoute('/users', [
      opOperation('get', { responses: [opResponse(200, arrayType(refType('User')), 'application/json')] }),
    ]);
    const postRoute = opRoute('/posts', [
      opOperation('get', { responses: [opResponse(200, arrayType(refType('Post')), 'application/json')] }),
    ]);
  ...
  it('puts models referenced by multiple tags in shared.ck', () => {
    ...
    const route1 = opRoute('/users', [
      opOperation('get', { query: 'Pagination', responses: [] }),
    ]);
    const route2 = opRoute('/posts', [
      opOperation('get', { query: 'Pagination', responses: [] }),
    ]);
