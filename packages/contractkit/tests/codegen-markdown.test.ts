...
describe('generateMarkdown', () => {
  ...
  describe('table of contents', () => {
    ...
    it('lists models in TOC', () => {
      const dto = dtoRoot([model('User', [field('id', scalarType('uuid'))]), model('Order', [field('id', scalarType('uuid'))])]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
    ...
    it('groups models by area in collapsible TOC section', () => {
      const dto = dtoRoot([model('LedgerAccount', [field('id', scalarType('uuid'))])], 'ledger.dto');
  ...
  describe('unified attributes table', () => {
    ...
    it('resolves ref query type to model fields', () => {
      const dto = dtoRoot([model('Pagination', [field('page', scalarType('int'))])]);
    ...
    it('resolves intersection query to flattened fields', () => {
      const dto = dtoRoot([model('Pagination', [field('page', scalarType('int'), { description: 'Page number' })])]);
      const query = intersectionType(refType('Pagination'), inlineObjectType([field('status', enumType('active', 'inactive'), { optional: true })]));
  ...
  describe('models', () => {
    ...
    it('renders model description as blockquote', () => {
      const dto = dtoRoot([model('User', [field('id', scalarType('uuid'))], { description: 'A user object' })]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
      expect(output).toContain('> A user object');
    });

    it('renders extends link for base model', () => {
      const dto = dtoRoot([model('Admin', [field('role', scalarType('string'))], { base: 'User' })]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
    ...
    it('marks optional fields as not required', () => {
      const dto = dtoRoot([model('User', [field('bio', scalarType('string'), { optional: true })])]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
    ...
    it('includes field description text', () => {
      const dto = dtoRoot([model('User', [field('name', scalarType('string'), { description: 'The user name' })])]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
      expect(output).toContain('The user name');
    });

    it('renders type alias as code block', () => {
      const dto = dtoRoot([model('Status', [], { type: enumType('active', 'inactive') })]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
      expect(output).toContain("type Status = 'active' | 'inactive'");
    });

    it('includes nullable modifier', () => {
      const dto = dtoRoot([model('User', [field('bio', scalarType('string'), { nullable: true, optional: true })])]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
  ...
  describe('formatting', () => {
    ...
    it('escapes pipe characters in enum types within tables', () => {
      const dto = dtoRoot([model('User', [field('status', enumType('active', 'inactive'))])]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
    ...
    it('escapes pipe characters in union types within tables', () => {
      const dto = dtoRoot([model('User', [field('value', unionType(scalarType('string'), scalarType('number')))])]);
      const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
...
describe('route modifiers', () => {
  describe('internal', () => {
    ...
    it('operation-level override on internal route makes that operation visible', () => {
      const op = opRoot([
        opRoute(
          '/admin/users',
          [opOperation('get', { modifiers: ['deprecated'], responses: [opResponse(200)] }), opOperation('post', { responses: [opResponse(201)] })],
          undefined,
          ['internal'],
        ),
      ]);
  ...
  describe('deprecated', () => {
    ...
    it('cascades route-level deprecated to operations', () => {
      const op = opRoot([
        opRoute('/users', [opOperation('get', { responses: [opResponse(200)] }), opOperation('post', { responses: [opResponse(201)] })], undefined, [
          'deprecated',
        ]),
      ]);
      const output = generateMarkdown({ dtoRoots: [], opRoots: [op] });
...
describe('model filtering by public reachability', () => {
  it('excludes models only referenced by internal operations', () => {
    const dto = dtoRoot([model('WebhookPayload', [field('event', scalarType('string'))])]);
    const op = opRoot([
      opRoute(
        '/webhooks/internal',
        [
  ...
  it('includes models referenced by at least one public operation', () => {
    const dto = dtoRoot([model('User', [field('id', scalarType('string'))])]);
  ...
  it('includes models reachable transitively through a public op reference', () => {
    const dto = dtoRoot([model('Order', [field('item', refType('OrderItem'))]), model('OrderItem', [field('sku', scalarType('string'))])]);
  ...
  it('shows all models when no .op files are present', () => {
    const dto = dtoRoot([model('Standalone', [field('x', scalarType('string'))])]);
    const output = generateMarkdown({ dtoRoots: [dto], opRoots: [] });
    expect(output).toContain('Standalone');
  });

  it('keeps public model when shared by both internal and public ops', () => {
    const dto = dtoRoot([model('SharedModel', [field('id', scalarType('string'))])]);
    const op = opRoot([
