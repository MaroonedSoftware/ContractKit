...
describe('generateOpenApi', () => {
  ...
  describe('component schemas', () => {
    ...
    it('handles description on models and fields', () => {
      const dto = dtoRoot([model('User', [field('name', scalarType('string'), { description: 'The user name' })], { description: 'A user object' })]);
      const output = generateOpenApi({
        dtoRoots: [dto],
        opRoots: [],
        config: {},
...
describe('route modifiers', () => {
  ...
  describe('schema filtering — internal operations', () => {
    it('excludes schemas only referenced by internal operations', () => {
      const dto = dtoRoot([model('PublicModel', [field('id', scalarType('uuid'))]), model('InternalModel', [field('secret', scalarType('string'))])]);
      const op = opRoot([
