...
/** Options every target that groups pages by area accepts. */
export interface AreaGroupingConfig {
    /**
     * Display names for area groups, keyed by the `area` meta value: `{ "openai": "OpenAI" }`.
     * Areas left out are humanized (`bankConnections` → "Bank Connections"). Merged over the
     * plugin-level `areaLabels`, so a target only needs the entries it changes.
     */
    areaLabels?: Record<string, string>;
}

/** A single self-contained Markdown API reference, readable without a build step. */
export interface MarkdownConfig extends AreaGroupingConfig {
...
export interface MintlifyConfig extends AreaGroupingConfig {
 ...
 */
export interface DocusaurusConfig extends AreaGroupingConfig {
...
export interface DocsPluginConfig {
    /**
     * Display names for area groups, shared by every target: `{ "openai": "OpenAI" }`. A target's
     * own `areaLabels` is merged over this one.
     */
    areaLabels?: Record<string, string>;
