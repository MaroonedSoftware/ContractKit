...
export function mountMcp(router: ReturnType<typeof ServerKitRouter>): void {
    router.post('/mcp', requirePolicy({ policy: false }), bodyParserMiddleware(['json']), async ctx => {
        ...
        const context = createMcpRequestContext({ requestId: ctx.requestId, logger: ctx.logger, authenticationSession: ctx.authenticationSession, container: ctx.container });
