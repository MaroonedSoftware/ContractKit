...
A DSL for API contracts. `.ck` files compile to Zod schemas, Koa or Fastify routers, TypeScript SDK
...
- **A grammar change touches every plugin.** The TypeScript plugin is not the only consumer
  of the AST — Python, Bruno, `openapi-to-ck`, and all four `plugin-docs` targets (openapi,
  markdown, mintlify, docusaurus) each need codegen and tests updated. The mintlify and
  docusaurus targets share the markdown target's renderer through the `MarkdownDialect` seam.
  See the `ck-grammar-change` skill.
