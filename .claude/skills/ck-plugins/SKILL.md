...
Swift SDK does the same against Swift's reserved words. The C# SDK spells it `PascalCase` with an
`Async` suffix and `@`-escapes a *parameter* that lands on a C# keyword (a PascalCase method name
never needs it).
