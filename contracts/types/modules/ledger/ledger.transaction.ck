...
contract LedgerTransaction: { # Represents a ledger transaction
    ...
    version: readonly int # The transaction version
    idempotencyKey: writeonly uuid # A unique idempotency key for the transaction
    status: enum(pending, posted) # The initial transaction status
    effectiveAt?: datetime # Effective date-time (defaults to now)
    description?: string(max=1024) | null # The description of the ledger transaction
...
contract LedgerTransactionEntry: { # Represents a ledger transaction entry
    ...
    effectiveAt: readonly datetime # When the entry takes effect
    ...
    status: readonly enum(pending, posted, archived) # The entry status
...
contract LedgerEntry: { # Represents a ledger entry
    ...
    accountVersion: readonly int # The account version at time of entry
    amount: readonly bigint(min=0) # The entry amount in smallest currency units
    status: readonly enum(pending, posted, archived) # The entry status
    direction: readonly enum(credit, debit) # The direction of the entry
    effectiveAt: readonly datetime # When the entry takes effect
    createdAt: readonly datetime # When the entry was created
    archivedAt?: readonly datetime # When the entry was archived
}
...
contract LedgerTransfer: { # Represents a ledger transfer
    ...
    amount: bigint(min=0) # The amount of the ledger transfer
    direction: enum(credit, debit) # The direction of the ledger transfer
    status?: enum(pending, posted) # The status of the ledger transfer
    effectiveAt?: datetime # The effective date of the ledger transfer
...
contract LedgerTransferMultiple: { # Represents a ledger transfer multiple
    ...
    effectiveAt?: datetime # The effective date of the ledger transfer multiple
    description?: string(max=1024) # The description of the ledger transfer multiple
}