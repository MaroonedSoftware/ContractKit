...
describe('getCompletions', () => {
  ...
  describe('modifier completions', () => {
    it('offers internal and deprecated after route path + colon (top-level)', () => {
      ...
      const index = new WorkspaceIndex();
      const items = getCompletions({ textDocument: { uri: doc.uri }, position: { line: 0, character: 14 } }, doc, index);
    ...
    it('route modifier completions are kind Keyword', () => {
      ...
      const index = new WorkspaceIndex();
      const items = getCompletions({ textDocument: { uri: doc.uri }, position: { line: 0, character: 8 } }, doc, index);
