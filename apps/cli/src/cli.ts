...
async function main() {
  ...
  const run = async () => {
    ...
    const sdkFiles = config.sdk ? await resolveFiles([...(config.sdk.types?.include ?? []), ...(config.sdk.clients?.include ?? [])], sdkBase) : [];
    ...
    const sdkEntryPath = config.sdk?.output ? join(sdkBase, resolveTemplate(config.sdk.output, { name: sdkName ?? 'sdk' })) : join(sdkBase, 'sdk.ts');
